...
function underlineText(name_list) {
    ...
    var x = document.getElementsByClassName('special-links');
    for (var i = 0; i < x.length; i++) {
        (function (i) {
            var name = name.split(' ')[0] + '%20' + name.split(' ')[1];
            jQuery.ajax(
                {
                    type: "GET",
                    url: "https://politify.herokuapp.com/congress/" + name,
                    success: function (request) { x[i].setAttribute('onclick', "nhpup.popup($('#" + div + "').html(), {'width': 1600});"); }
                });
        })(i);
    }
}

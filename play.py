...
def get_d(name):
    ...
    call = 'https://www.bing.com/news/search?q=' + joined_name
    print(call)
    r = requests.get(call)
    parsed = BS(r.text)
    x = [y.text for y in parsed.find_all('a', {'class': 'title'})][:7]
    d['news'] = x
